...
def get_creative_work(tmdb_id,media_type):
    ...
    #To create the director person we need the TMDB_id of the person and their name filtered out based on job title.
    subject_info['Person']=get_crew(credits,"Director")
    ...
    subject_info['CreativeWork'] = item

    return subject_info

    # subject_info['media_type'] = media_type
    # subject_info['type'] = media_types[media_type]
    #Save this line for later. May be a better way to get categories.
...
def get_crew(credits,role):
  ...
  return [{'tmdb_id':item['id'],'name':item['name']} for item in credits['crew'] if item["job"] == role]
